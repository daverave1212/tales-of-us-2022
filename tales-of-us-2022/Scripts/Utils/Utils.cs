...
public static class Utils
{
	...
	public static bool IsMouseLeftClicked(InputEvent @event)
	{
		...
		return false;
		/*
		if (inputEvent.GetType() == typeof(InputEventMouseButton) == false)
			return false;
		var inputEventMouse = (InputEventMouseButton) inputEvent;
		return inputEventMouse.Pressed && inputEventMouse.ButtonIndex == ButtonLeft;
		*/
	}
	public static bool IsMouseLeftReleased(InputEvent @event)
	{
		if (@event is InputEventMouseButton btn && btn.ButtonIndex == (int)ButtonList.Left && (@event.IsPressed() == false)) {
			return true;
		}
		return false;
		/*
